...
select sub.plan_id, plan_name, count(distinct customer_id) as events
from foodie_fi.subscriptions as sub
join 
foodie_fi.plans as plan
...
round((count(distinct customer_id)*100.0)/(select count(distinct customer_id) from foodie_fi.subscriptions),1) as churn_perct
from foodie_fi.subscriptions 
...
with next_plan as (
  ...
  select count(distinct customer_id) as churn_after_trial_count,
  round(count(distinct customer_id)*100.0/(select count(distinct customer_id) from foodie_fi.subscriptions),2) as churn_percentage
  from next_plan
...
previous_plan_trial as (select * from next_plan 
          where plan_id = 0)
          
 select next_plan_id, plan_name, 
 ...
 from previous_plan_trial as previous
 join 
 foodie_fi.plans as p
 on previous.next_plan_id = p.plan_id
